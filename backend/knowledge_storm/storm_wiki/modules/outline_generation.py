...
class StormOutlineGenerationModule(OutlineGenerationModule):
    ...
    def __init__(self, outline_gen_lm: Union[dspy.dsp.LM, dspy.dsp.HFModel], report_language: str):
        super().__init__()
    ...
    def generate_outline(
        ...
        Args:
            ...
            report_language (str): Language of the report
        ...
        with dspy.settings.context(lm=self.engine):
            if old_outline is None:
                old_outline = ArticleTextProcessing.clean_up_outline(
                    self.draft_page_outline(topic=topic, report_language=report_language).outline
                )
                if callback_handler:
            ...
            outline = ArticleTextProcessing.clean_up_outline(
                self.write_page_outline(
                    topic=topic, report_language=report_language,old_outline=old_outline, conv=conv
                ).outline
            )
            if callback_handler:
...
class WritePageOutline(dspy.Signature):
    ...
    4. Write the outline in the language specified in the input.

    """

    topic = dspy.InputField(prefix="The topic you want to write: ", format=str)
    report_language = dspy.InputField(prefix="Language: ", format=str)
    outline = dspy.OutputField(
        prefix="Write the Wikipedia page outline in the specified language:\n", 
        format=str
    )
...
class NaiveOutlineGen(dspy.Module):
    ...
    def forward(self, topic: str, report_language: str):
        outline = self.write_outline(topic=topic, report_language=report_language).outline
...
class WritePageOutlineFromConv(dspy.Signature):
    ...
    outline = dspy.OutputField(
        prefix='Write the Wikipedia page outline in the specified language (Use "#" Title" to indicate section title, "##" Title" to indicate subsection title, ...):\n',
        format=str,
    )
