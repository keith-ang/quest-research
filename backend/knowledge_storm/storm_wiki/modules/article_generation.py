...
class StormArticleGenerationModule(ArticleGenerationModule):
    ...
    def generate_section(
        self, topic, section_name, information_table, section_outline, section_query, report_language
    ):
...
class ConvToSection(dspy.Module):
    ...
    def forward(
        self, topic: str, outline: str, section: str, collected_info: List[Information], report_language: str
    ):
        info = ""
        ...
        with dspy.settings.context(lm=self.engine):
            section = ArticleTextProcessing.clean_up_section(
                self.write_section(topic=topic, info=info, section=section, report_language=report_language).output
            )
...
class WriteSection(dspy.Signature):
    ...
    Here is the format of your writing:
        ...
        4. Write the section in the language specified in the input.
    """
    ...
    output = dspy.OutputField(
        prefix="Write the section in the specified language with proper inline citations (Start your writing with # section title. Don't include the page title or try to write other sections):\n",
        format=str,
    )
